...
__END_COMMENTARY__
    ...
    it 'can parse instruction with single operand' do
      warrior = Warrior.new "org foo"
      warrior.should_not be_nil
      warrior.instructions.first[:operation][:opcode].should == :org
      warrior.instructions.first[:a].should == :foo
    end
    
    it 'can parse instruction with two operands' do
      warrior = Warrior.new "mov 0,1"
      warrior.should_not be_nil
      warrior.instructions.first[:operation][:opcode].should == :mov
